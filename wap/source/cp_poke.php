...
$_SGLOBAL['db']->query("DELETE \x46\122\117\115 ".tname('poke')." WHERE uid='$_SGLOBAL[supe_uid]' AND fromuid='$uid'"); $_SGLOBAL['db']->query("UPDATE ".tname('space')." SET pokenum=pokenum-1 WHERE uid='$_SGLOBAL[supe_uid]' AND pokenum>0"); }  
getreward('poke', 1, 0, $uid);  
updatestat('poke'); showmessage('poke_success', $_POST['refer'], 1, array($_SN[$tospace['uid']])); } } elseif($op == 'ignore') { $where = empty($uid)?'':"AND fromuid='$uid'"; $_SGLOBAL['db']->query("DELETE \106\122\x4f\x4d ".tname('poke')." WHERE uid='$_SGLOBAL[supe_uid]' $where");  
