...
if($_GET['op'] == 'delete') {
	...
	if(submitcheck('sharesubmit')) {
		...
		if($type == 'link') {
			...
			if(preg_match("/(youku.com|youtube.com|5show.com|ku6.com|sohu.com|mofile.com|sina.com.cn|tudou.com|qq.com)$/i", $parseLink['host'], $hosts)) {	//Modify By 01
				$flashvar = getflash($link, $hosts[1]);
				$flashimg = getflashimg($link, $hosts[1]);	//Add By 01
				if(!empty($flashvar)) {
...
//Add By 01↑
			...
			// 判断是否 Flash
			if(preg_match("/\.swf$/i", $link) || strstr($link, ".swf?")) {
...
function getflash($link, $host) {
	...
	if('youku.com' == $host) {
		...
		//http://v.mofile.com/show/HLD5PLD2.shtml	//Add By01
		preg_match_all("/\/([\w\-]+)\.shtml/", $link, $matches);	//Add By01
...
//Add By 01↓
		...
		// http://video.qq.com/v1/videopl?v=6QFnVNFVQGL
		preg_match_all("/v\=([\w\-]+)/", $link, $matches);
		if(!empty($matches[1][0])) {
			$return = $matches[1][0];
		}
