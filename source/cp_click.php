...
//				'subject' => "<a href=\"space.php?uid=$item[uid]&do=blog&id=$item[blogid]\">$item[subject]</a>",
				'subject' => "<a href=\"viewspace.php?uid=$item[uid]&do=blog&id=$item[blogid]\">$item[subject]</a>",	//Modify By 01
				'click' => $click['name']
			);
			$note_type = 'clickblog';
//			$q_note = cplang('note_click_blog', array("space.php?uid=$item[uid]&do=blog&id=$item[blogid]", $item['subject']));
			$q_note = cplang('note_click_blog', array("viewspace.php?uid=$item[uid]&do=blog&id=$item[blogid]", $item['subject']));	//Modify By 01
			break;
		case 'tid':
...
//			$fs['image_links'] = array("space.php?uid=$item[uid]&do=album&picid=$item[picid]");
			$fs['image_links'] = array("viewspace.php?uid=$item[uid]&do=album&picid=$item[picid]");	//Modify By 01
			$fs['body_general'] = $item['title'];
			$note_type = 'clickpic';
//			$q_note = cplang('note_click_pic', array("space.php?uid=$item[uid]&do=album&picid=$item[picid]"));
			$q_note = cplang('note_click_pic', array("viewspace.php?uid=$item[uid]&do=album&picid=$item[picid]"));	//Modify By 01
			break;
	}
