...
if($tagname) {
	...
	$_TPL['css'] = 'thread';
	include_once template("space_sys_tagname");
	...
	$_TPL['css'] = 'thread';
	include_once template("space_sys_field");
	...
	if($_GET['view'] == 'list' || $_GET['view'] == 'digest') {
		...
		$_TPL['css'] = 'thread';
		include_once template("space_sys_list");
		...
		$_TPL['css'] = 'thread';
		include_once template("space_sys_member");
		...
		$_TPL['css'] = 'thread';
		include_once template("space_sys_event");
		...
		$_TPL['css'] = 'thread';
		include_once template("space_sys_index");
	}
	...
	$threadlist = array();
	if($tagids) {
		$query = $_SGLOBAL['db']->query("SELECT * FROM ".tname('thread')." as `thread` LEFT JOIN ".tname('mtag')." AS `mtag` on `thread`.`tagid`=`mtag`.`tagid`  WHERE `mtag`.`fieldid`<'11' and `thread`.`tagid` IN (".simplode($tagids).") ORDER BY dateline DESC LIMIT 0,10");
		while ($value = $_SGLOBAL['db']->fetch_array($query)) {
			realname_set($value['uid'], $value['username']);
			realname_set($value['lastauthorid'], $value['lastauthor']);
	...
	$_TPL['css'] = 'thread';
	include_once template("space_sys");
}
