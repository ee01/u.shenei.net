...
/*
	...
	$Id: space_blog.php 10322 2008-11-28 09:44:18Z liguode $
*/
...
$scss='<link type="text/css" rel="stylesheet" href="viewspace/css/'.get_userURL2($space[uid]).'?'.$usercss->date.'" />';
}
else
{
$value=Usercssabout($uid);
$wrap=strlen($value[frame_set])==2?wraptwo:wrap;
$scss='<link type="text/css" rel="stylesheet" href="viewspace/css/'.get_userURL2($space[uid]).'" />';
}
...
if($id) {
	...
	//日志分类 Add By 01
	$classquery = $_SGLOBAL['db']->query("SELECT classid, classname FROM ".tname('class')." WHERE uid='$space[uid]'");
	while ($classvalue = $_SGLOBAL['db']->fetch_array($classquery)) {
		$classarr[$classvalue['classid']] = $classvalue['classname'];
	}
		...
		if(!empty($_SCONFIG['uc_tagrelated']) && $_SCONFIG['uc_status']) {
			...
			if($tag_blogids) {
				...
				while ($value = $_SGLOBAL['db']->fetch_array($query)) {
					...
					$value['url'] = "viewspace.php?uid=$value[uid]&do=blog&id=$value[blogid]";
	...
	//分页
	$multi = multiviewspace($count, $perpage, $page, "viewspace.php?uid=$blog[uid]&do=$do&id=$id", '', 'content');
	...
	//格式化变量
	$title= $blog['subject'];
	$link = "http://".$_SERVER["HTTP_HOST"].($_SERVER["REQUEST_URI"]?$_SERVER["REQUEST_URI"]:($_SERVER["QUERY_STRING"]?'?'.$_SERVER["QUERY_STRING"]:$_SERVER["PHP_SELF"]));
	if($_SCONFIG['allowrewrite']) {
		$link = str_replace('.php?rewrite=','-',$link ).".html";	
	}
	$tuilist=$sharelist=$tuishare = array();
	$query = $_SGLOBAL['db']->query("SELECT * FROM ".tname('tuishare')."
		ORDER BY sorder DESC
		LIMIT 0,18");
	while ($value = $_SGLOBAL['db']->fetch_array($query)) {
		$value['appurl'] = str_replace('{$link}',urlencode($link),$value['appurl'] );
		$value['appurl'] = str_replace('{$title}', urlencode($title),$value['appurl'] );
		$tuishare[$value['tuid']][] = $value;
	}
	//筛选类型
	foreach ($tuishare as $tuivalue) {
		$a_value=array();
		foreach ($tuivalue as $value) {
			if(empty($a_value)) {
				$a_value = $value;
			}
		}
	...
	include_once template("newsp_viewspace_blog_view");
	...
	if($_GET['view'] == 'click') {
		...
		if($_GET['view'] == 'all') {
			...
			if($_GET['orderby']) {
				...
				$theurl = "viewspace.php?uid=$space[uid]&do=blog&view=all&orderby=$_GET[orderby]";
	...
	include_once template("newsp_viewspace_blog");
}
