...
//温馨提示
//$aid = $_SGLOBAL['db']->result($_SGLOBAL['db']->query("SELECT id  FROM ".tname('attention')." WHERE startime<=".$_SGLOBAL['timestamp']." AND endtime>=".$_SGLOBAL['timestamp']." AND isactive=1 ORDER BY aorder DESC LIMIT 1"),0);
//$aid = rand(1,$aid);	//Add By 01
$aids = array();
$query = $_SGLOBAL['db']->query("SELECT id  FROM ".tname('attention')." WHERE startime<=".$_SGLOBAL['timestamp']." AND endtime>=".$_SGLOBAL['timestamp']." AND isactive=1 ORDER BY id DESC");
while($value = $_SGLOBAL['db']->fetch_array($query)) {
	$aids[] = $value;
}
$aid = $aids[rand(0,count($aids)-1)][id];
...
if($space['self'] && empty($start)) {
	...
	$task = gettask();
	$task_getreward = getmytask(7); //8为TASKID号	打卡领工资 Add By 01
...
foreach ($list as $date=>$item) {
	...
	foreach ($item as $data) {
		$pv['user'] = $_SN[$data[uid]];
		if ($data['title_data']['message']) $pv['message'] = $data['title_data']['message'];
		else {
			$explode = explode('</a>', $data['title_template']);
			$explode = explode($explode[0].'</a>', $data['title_template']);
			$pv['message'] = $explode[1];
		}
		$pv['date'] = $data['dateline'];
		$pvdata[] = $pv;
		$i++;
	}
	if ($i > 10) break;
...
//echo '<iframe noscroll border="0" src="sync.php?app=qq&sync_id='.$_SGLOBAL['supe_uid'].'" width="0" height="0" name="stat" id="stat"></iframe>';
//if (rand(0,1)) {
	sync('sina');
//}else{
	sync('qq');
