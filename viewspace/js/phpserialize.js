 ...
 * @version             2.2, tested on FF 1.0, 1.5, IE 5, 5.5, 6, 7 beta 2, Opera 8.5, Konqueror 3.5, Safari 2.0.3
	...
	aaa['222'] = 'serialize:设置是否序列化该属性 deserialize:设置是否反序列化该属性。 format:设置用于格式化输出、解析日';
	aaa['lksdf'] = '12345';
	var bbb = php.serialize(aaa);
	var ccc = php.unserialize(bbb);
	for(var i in ccc) {
		document.write('\''+i+' => '+ccc[i]+'<br>');
	}
	document.write(php.serialize(aaa));
 */
function PHP_Serializer(stype) {
	...
	function __uStringGBK() {
		...
		while(sli) {
			c = __s.charCodeAt(vls);
			pos += (c < 128) ? 1 : ((c < 2048) ? 2 : ((c < 65536) ? 2 : 4));
			++vls;
			if(pos === sli)
				sli = 0;
		};
