...
if(empty($space['feedfriend'])) {
	...
	foreach ($feed_list as $values) {
		...
		foreach ($values as $value) {
			...
			$actors[] = "<a target=\"mainyinyue\"  href=\"javascript:;\" onclick=\"javascript:spaceCategory('viewspace.php?uid=$value[uid]', '');\">".$_SN[$value['uid']]."</a>";
		}
...
function ckicon_uid($feed) {
	...
	$is_myapp = is_numeric($feed['icon'])?1:0;
	...
	if(empty($_GET['filter']) && $is_myapp && empty($_SGLOBAL['my_userapp'][$feed['icon']])) {
		//最多显示个数
