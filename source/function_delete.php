...
//温馨提示删除
function deleteattention($adids) {
	global $_SGLOBAL;

	if(!checkperm('managead')) return false;

	$dels = $newids = array();
	$query = $_SGLOBAL['db']->query("SELECT * FROM ".tname('attention')." WHERE id IN (".simplode($adids).")");
	while ($value = $_SGLOBAL['db']->fetch_array($query)) {
	...
	//数据
	$_SGLOBAL['db']->query("DELETE FROM ".tname('attention')." WHERE id IN (".simplode($newids).")");
...
//推广收藏分享删除 Add By 01
function deletetuishare($adids) {
	global $_SGLOBAL;

	if(!checkperm('managead')) return false;

	$dels = $newids = array();
	$query = $_SGLOBAL['db']->query("SELECT * FROM ".tname('tuishare')." WHERE tuid IN (".simplode($adids).")");
	while ($value = $_SGLOBAL['db']->fetch_array($query)) {
	...
	//数据
	$_SGLOBAL['db']->query("DELETE FROM ".tname('tuishare')." WHERE tuid IN (".simplode($newids).")");
