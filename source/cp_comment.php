...
if(submitcheck('commentsubmit')) {
	...
	switch ($idtype) {
		...
		case 'ND_coment_id':
			...
			$hotarr = array('id', $blog['id'], $blog['hotuser']);
			$stattype = 'blogcomment';//统计
			break;
		case 'sid':
			//读取日志
	...
	if(!comment_hidden($cid,$_SGLOBAL['supe_uid'],1)) {
		showmessage('can_not_reply_hidden_comments');
	}
	...
	switch ($_POST['idtype']) {
		case 'uid':
			...
			$fs['title_template_self'] = cplang('{actor} 在 自己 的留言板上自言自语');	//Add By 01
		...
		case 'picid':
			...
			$fs['title_template_self'] = cplang('{actor} 评论了 自己 的图片');	//Add By 01
			$fs['title_data'] = array('touser'=>"<a href=\"space.php?uid=$tospace[uid]\">".$_SN[$tospace['uid']]."</a>");
...
//			$fs['image_links'] = array("space.php?uid=$tospace[uid]&do=album&picid=$pic[picid]");
			$fs['image_links'] = array("viewspace.php?uid=$tospace[uid]&do=album&picid=$pic[picid]");	//Modify By 01
		...
		case 'blogid':
			...
			$fs['title_template_self'] = cplang('{actor} 评论了 自己 的日志 {blog}');	//Add By 01
//			$fs['title_data'] = array('touser'=>"<a href=\"space.php?uid=$tospace[uid]\">".$_SN[$tospace['uid']]."</a>", 'blog'=>"<a href=\"space.php?uid=$tospace[uid]&do=blog&id=$id\">$blog[subject]</a>");
			$fs['title_data'] = array('touser'=>"<a href=\"space.php?uid=$tospace[uid]\">".$_SN[$tospace['uid']]."</a>", 'blog'=>"<a href=\"viewspace.php?uid=$tospace[uid]&do=blog&id=$id\">$blog[subject]</a>");	//Modify By 01
			$fs['body_template'] = '';
			$fs['body_data'] = array();
			$fs['body_general'] = '';
		...
		case 'ND_coment_id':
			//更新评论统计
			$_SGLOBAL['db']->query("UPDATE ".tname('nd_data')." SET comment=comment+1 WHERE id='$id'");
			//事件
			$fs['title_template'] = cplang('{actor} 发表了一篇资源评论');
			$fs['title_template_self'] = cplang('{actor} 对自己的资源进行了评论');	//Add By 01
			$fs['title_data'] = array('touser'=>"<a href=\"space.php?uid=$tospace[uid]\">".$_SN[$tospace['uid']]."</a>", 'blog'=>"<a href=\"ND_upload.php?do=info&id=$id\">$blog[title]</a>");
			$fs['body_template'] = '';
			$fs['body_data'] = array();
			$fs['body_general'] = '';
			$fs['target_ids'] = '';
		...
		case 'sid':
			...
			$fs['title_template_self'] = cplang('{actor} 对 自己 分享的 {share} 发表了评论');	//Add By 01
		...
		case 'eventid':
			...
			$fs['title_template_self'] = cplang('{actor} 在 自己 组织的活动 {event} 中留言了');	//Add By 01
		...
		case 'pid':
			...
			$fs['title_template_self'] = cplang('{actor} 评论了 自己 的投票 {poll}');	//Add By 01
//			$fs['title_data'] = array('touser'=>"<a href=\"space.php?uid=$tospace[uid]\">".$_SN[$tospace['uid']]."</a>", 'poll'=>"<a href=\"space.php?uid=$tospace[uid]&do=poll&pid=$id\">$poll[subject]</a>");
			$fs['title_data'] = array('touser'=>"<a href=\"space.php?uid=$tospace[uid]\">".$_SN[$tospace['uid']]."</a>", 'poll'=>"<a href=\"viewspace.php?uid=$tospace[uid]&do=poll&pid=$id\">$poll[subject]</a>");	//Modify By 01
			$fs['body_template'] = '';
			$fs['body_data'] = array();
			$fs['body_general'] = '';
			$fs['friend'] = '';
...
//		'replyid' => empty($_POST['replyid'])?0:intval($_POST['replyid']),	//Add By 01
		'replyid' => empty($_POST['cid'])?0:intval($_POST['cid']),	//Add By 01
...
//			$n_url = "space.php?uid=$tospace[uid]&do=wall&cid=$cid";
			$n_url = "viewspace.php?uid=$tospace[uid]&do=wall&cid=$cid";	//Modify By 01
			$note_type = 'wall';
...
//			$n_url = "space.php?uid=$tospace[uid]&do=album&picid=$id&cid=$cid";
			$n_url = "viewspace.php?uid=$tospace[uid]&do=album&picid=$id&cid=$cid";	//Modify By 01
...
//			$n_url = "space.php?uid=$tospace[uid]&do=blog&id=$id&cid=$cid";
			$n_url = "viewspace.php?uid=$tospace[uid]&do=blog&id=$id&cid=$cid";	//Modify By 01
...
//			$n_url = "space.php?uid=$tospace[uid]&do=poll&pid=$id&cid=$cid";
			$n_url = "viewspace.php?uid=$tospace[uid]&do=poll&pid=$id&cid=$cid";	//Modify By 01
	...
	if(empty($comment)) {
		...
		//Add By 01↓
			...
			//事件发布
			if(ckprivacy('comment', 1)) {
				feed_add($fs['icon'], $fs['title_template_self'], $fs['title_data'], $fs['body_template'], $fs['body_data'], $fs['body_general'],$fs['images'], $fs['image_links'], $fs['target_ids'], $fs['friend']);
			}
		}
		//Add By 01↑
...
//		updatetable('comment', array('message'=>$message), array('cid'=>$comment['cid']));
		updatetable('comment', array('message'=>$message, 'hiddenreply'=>empty($_POST['hiddenreply'])?0:1), array('cid'=>$comment['cid']));	//Modify By 01
