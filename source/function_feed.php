...
//					$url = "space.php?uid=$value[uid]&do=blog&id=$value[blogid]";
					$url = "viewspace.php?uid=$value[uid]&do=blog&id=$value[blogid]";	//Modify By 01
...
//								'album' => "<a href=\"space.php?uid=$value[uid]&do=album&id=$value[albumid]\">$value[albumname]</a>",
								'album' => "<a href=\"viewspace.php?uid=$value[uid]&do=album&id=$value[albumid]\">$value[albumname]</a>",	//Modify By 01
								'picnum' => $value['picnum']
							);
						}
...
//						$setarr['image_'.$key.'_link'] = "space.php?uid=$value[uid]&do=album&picid=$value[picid]";
						$setarr['image_'.$key.'_link'] = "viewspace.php?uid=$value[uid]&do=album&picid=$value[picid]";	//Modify By 01
						$key++;
					} else {
						break;
					}
...
//					$url = "space.php?uid=$value[uid]&do=album&picid=$value[picid]";
					$url = "viewspace.php?uid=$value[uid]&do=album&picid=$value[picid]";	//Modify By 01
...
//				$url = "space.php?uid=$value[uid]&do=poll&pid=$value[pid]";
				$url = "viewspace.php?uid=$value[uid]&do=poll&pid=$value[pid]";	//Modify By 01
...
//Add By 01↓
			if($setarr['icon'] == 'blog' && ($_SGLOBAL['timestamp']-$setarr['dateline']) > 3600) {
				$setarr['title_template'] = "{actor} 修改更新了".(($_SGLOBAL['timestamp']-$setarr['dateline'])>259200?'曾经写的':'')."一篇".($setarr['body_data']=='N;'?'加密日志 {subject}':'日志');
