...
if($pid) {
	...
	if($poll['expiration'] && $poll['expiration'] < $_SGLOBAL['timestamp']) {
		...
		if(empty($poll['summary']) && !$poll['notify']) {
			...
			$note = cplang('note_poll_finish', array("viewspace.php?uid=$poll[uid]&do=poll&pid=$poll[pid]", $poll['subject']));
	...
	if($isfriend) {
		...
		//分页
		$multi = multiviewspace($count, $perpage, $page, "viewspace.php?uid=$poll[uid]&do=$do&pid=$pid", '', 'div_main_content');
	}
	//取出最新投票
	...
	include_once template("newsp_viewspace_poll_view");
	...
	if($_GET['view'] == 'new') {
		...
		$theurl = "viewspace.php?uid=$space[uid]&do=$do&view=new";
		...
		$theurl = "viewspace.php?uid=$space[uid]&do=$do&view=hot";
		
	} elseif($_GET['view'] == 'friend') {
		
		$indexsql = 'USE INDEX (dateline)';
		$wherearr[] = "p.uid IN ($space[feedfriend])";
		$theurl = "viewspace.php?uid=$space[uid]&do=$do&view=friend";
		...
		$theurl = "viewspace.php?uid=$space[uid]&do=$do&view=reward";
		...
		$filtrate = array($_GET['filtrate']=>' class="active"');
		$theurl = "viewspace.php?uid=$space[uid]&do=$do&view=me&filtrate=".$_GET['filtrate'];
	...
	include_once template("newsp_viewspace_poll_list");
}
