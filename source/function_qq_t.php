...
class MBOpenTOAuth {
    ...
    function __construct($consumer_key, $consumer_secret, $oauth_token = NULL, $oauth_token_secret = NULL) { 
        $this->sha1_method = new OAuthSignatureMethod_HMAC_SHA1_qq(); 
        $this->consumer = new OAuthConsumer_qq($consumer_key, $consumer_secret); 
        if (!empty($oauth_token) && !empty($oauth_token_secret)) { 
            $this->token = new OAuthConsumer_qq($oauth_token, $oauth_token_secret); 
        } else { 
            $this->token = NULL; 
        } 
	}
    ...
    function getRequestToken($oauth_callback = NULL) { 
		...
		$token = OAuthUtil_qq::parse_parameters($request); 
        $this->token = new OAuthConsumer_qq($token['oauth_token'], $token['oauth_token_secret']); 
        return $token; 
    } 
	...
	* Exchange the request token and secret for an access token and 
    ...
    function oAuthRequest($url, $method, $parameters , $multi = false) { 
        ...
        $request = OAuthRequest_qq::from_consumer_and_token($this->consumer, $this->token, $method, $url, $parameters); 
	...
	function http($url, $method, $postfields = NULL , $multi = false){
		...
		if($multi){
			$header .= "Content-Type: multipart/form-data; boundary=" . OAuthUtil_qq::$boundary . "\r\n";	
		}else{	
	...
	function http($url, $method, $postfields = NULL , $multi = false){
        ...
        $header_array2=array(); 
        if( $multi ) 
        	$header_array2 = array("Content-Type: multipart/form-data; boundary=" . OAuthUtil_qq::$boundary , "Expect: ");
