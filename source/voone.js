...
//		shownum：显示的多少个数字
//		showobj：显示的组件
function showpages1(total,method,page,pernum,shownum,showobj){
    if(isNaN(total) || total < 1){
		$(showobj).style.display="none";
		return;
	}
	...
	var pageup = parseInt(page) - 1;
	var pagenext = parseInt(page) + 1;
	var up = "<a href=\"javascript:eval('"+ method + "(" + pageup + ")');\"><img src=\"/images/button_prev.gif\" border=\"0\" /></a>";
	var next = "<a href=\"javascript:eval('"+ method + "(" + pagenext + ")');\"><img src=\"/images/button_nexto.gif\" border=\"0\" /></a>";
	var tmp = "";
	if(page > 1)
		tmp = up + "&nbsp;&nbsp;&nbsp;";
	else
...
//      uid：userid
//		page：当前第几页
function nextpage(total,method,uid,page){
    var tmp = "";
    if(total<2){
        $("right_span"+uid).style.display = "none";
        return;
    }
    var pagenext = parseInt(page) + 1;
    if(page<total){
        var next = "<img src=\"/images/icon_next.gif\" border=\"0\" align=\"absmiddle\" style=\"border: none; padding: 0px; float: none;cursor:pointer;\" title=\"下一页\" onclick=\"javascript:eval('"
				+ method + "(" + pagenext + "," + uid + "," + total + ")');\" width=\"4\" height=\"7\" />";
	    tmp = next;
	    
	}
	$("right_span"+uid).innerHTML = tmp;
}
...
//      uid：userid
//		page：当前第几页
function uppage(total,method,uid,page){
    var tmp = "";
    var pageup = parseInt(page) - 1;
    if(page==1){
        $("left_span"+uid).innerHTML = "";
        return;
    }
        var up = "<img src=\"/images/icon_pre.gif\" border=\"0\" align=\"absmiddle\" style=\"border: none; padding: 0px; float: none;cursor:pointer;\" title=\"上一页\" onclick=\"javascript:eval('"
				+ method + "(" + pageup + ","+ uid + "," + total + ")');\" width=\"4\" height=\"7\" />";
	    tmp = up;
	    $("left_span"+uid).innerHTML = tmp;
}
//按照宽来等比例缩放
function imgload_w(simg,w)
{
	var t_img = new Image();
	t_img.src = simg.src;
	var t_w = t_img.width;
	var t_h = t_img.height;
	if(t_w<1 || t_h<1)
		return;
	if(t_w > w){
		t_img.height = t_h * w / t_w;
		t_img.width = w;
	}
	if(t_img.width < 1 || t_img.height < 1)
		return;
	simg.width = t_img.width;
	simg.height = t_img.height;
	simg.style.width = t_img.width + "px";
	simg.style.height = t_img.height + "px";
}
//按照高宽等比例缩放图片
function imgload(simg,w,h)
{
	var t_img = new Image();
	t_img.src = simg.src;
	var t_w = t_img.width;
	...
	if(t_w > w){
		...
		t_img.width = w;
		if(t_img.height > h){
			t_img.width = t_img.width * h / t_img.height;
			t_img.height = h;
		}
	}
	else if(t_h > h){
		t_img.width = t_w * h / t_h;
		t_img.height = h;
		if(t_img.width > w){
			t_img.height = t_img.height * w / t_img.width;
			t_img.width = w;
		}
	}
	if(t_img.width < 1 || t_img.height < 1)
...
function xml_table_data(){
	try{
		return arguments[0].getElementsByTagName(arguments[1])[0].lastChild.data;
	}catch(e){return "";}
}
//获取字符的长度，汉字算两个占位
