...
class WeiboOAuth { 
    ...
    function __construct($consumer_key, $consumer_secret, $oauth_token = NULL, $oauth_token_secret = NULL) { 
        $this->sha1_method = new OAuthSignatureMethod_HMAC_SHA1_sina(); 
        $this->consumer = new OAuthConsumer_sina($consumer_key, $consumer_secret); 
        if (!empty($oauth_token) && !empty($oauth_token_secret)) { 
            $this->token = new OAuthConsumer_sina($oauth_token, $oauth_token_secret); 
        } else { 
            $this->token = NULL; 
        } 
    } 
    ...
    function getRequestToken($oauth_callback = NULL) { 
        ...
        $token = OAuthUtil_sina::parse_parameters($request); 
        $this->token = new OAuthConsumer_sina($token['oauth_token'], $token['oauth_token_secret']); 
        return $token; 
    } 
    ...
    function oAuthRequest($url, $method, $parameters , $multi = false) { 
        ...
        // echo $url ; 
        $request = OAuthRequest_sina::from_consumer_and_token($this->consumer, $this->token, $method, $url, $parameters); 
...
*/
        ...
        $header_array2=array(); 
        if( $multi ) 
        	$header_array2 = array("Content-Type: multipart/form-data; boundary=" . OAuthUtil_sina::$boundary , "Expect: ");
