...
//		$query = $_SGLOBAL['db']->query("SELECT * FROM ".tname('thread')." WHERE tagid IN (".simplode($tagids).") ORDER BY dateline DESC LIMIT 0,10");
		$query = $_SGLOBAL['db']->query("SELECT * FROM ".tname('thread')." as `thread` LEFT JOIN ".tname('mtag')." AS `mtag` on `thread`.`tagid`=`mtag`.`tagid`  WHERE `mtag`.`fieldid`>'10' and `thread`.`tagid` IN (".simplode($tagids).") ORDER BY dateline DESC LIMIT 0,10");	//Modify By 01
		while ($value = $_SGLOBAL['db']->fetch_array($query)) {
			realname_set($value['uid'], $value['username']);
			realname_set($value['lastauthorid'], $value['lastauthor']);
