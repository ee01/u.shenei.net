...
function ajaxpostt(formid, showid, func) {
	showloading();
	if(ajaxpostHandle != 0) {
		return false;
	}
	...
	if(ajaxframe.attachEvent) {
		ajaxframe.detachEvent ('onload', ajaxpost_loada);
		ajaxframe.attachEvent('onload', ajaxpost_loada);
	} else {
		document.removeEventListener('load', ajaxpost_loada, true);
		ajaxframe.addEventListener('load', ajaxpost_loada, false);
	}
	$(formid).submit();
	return false;
}

function ajaxpost_loada() {
	showloading('none');
	if(is_ie) {
