 ...
 * 创建音乐
 */
function createMusic(e, show) {
	if(typeof show == 'undefined') {
		musictag = '';
		var sMusic = $('musicUrl').value;
		var sAuto = $('MusicAuto').value;
		if ((sMusic!=null) && (sMusic!="http://")){
			setCaret();
			if (sAuto == 0) {
				musictag = '[music=auto]';
			} else {
			...
			format("insertHTML", musictag + sMusic + '[/music]');
		}
		fHide($('createMusic'));
		$('musicUrl').value = 'http://';
	} else {
		...
		getCaret();
		var dvMusicBox = $("createMusic");
		var iX = e.clientX;
		var iY = e.clientY;
		dvMusicBox.style.display = "";
		dvMusicBox.style.left = (iX-300) + "px";
		dvMusicBox.style.top = 33 + "px";
	}
}
/**
 * 删除字符串两边空格
