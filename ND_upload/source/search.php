...
if(!empty($keyword))
{
	...
	$sql	=	"select nd.*,nt.name as ntt from ".tname('nd_data')." as nd left join ".tname('nd_type')." as nt on nt.id =nd.type where nd.title like '%".$keyword."%'  and nd.state>0 order by nd.".$orders." ".$dec;
	$actives 	= 	array('nd_alls'=>' class="active"');
	$perpage 	= 	10;
	$mpurl 		= 	'ND_upload.php?do=search&keyword='.urlencode($keyword);
	...
	while ($value= $_SGLOBAL['db']->fetch_array($query))
	{
		...
		$value['freinds']	=	explode(",",$spaces['friend']);
		$size				=	$value['size'];
		$daxiao1			=	$size/1024;
...
{
	showmessage('ÇëÌîÐ´ËÑË÷ÄÚÈÝ£¡','ND_upload.php',3);
}
include_once template("ND_upload/template/search");
?>
