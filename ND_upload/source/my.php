...
$config		= 	$_SGLOBAL['db']->fetch_array($query);
	...
	$sql 		= 	"select nd.*,nt.name as ntt from ".tname('nd_data')." as nd left join ".tname('nd_type')." as nt on nt.id =nd.type where nd.uid=".$_SGLOBAL['supe_uid']." order by nd.".$orders." ".$dec;
	$query 		= 	$_SGLOBAL['db']->query($sql);
...
include_once template("ND_upload/template/my");
?>
