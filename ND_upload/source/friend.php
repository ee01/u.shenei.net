...
if ( !defined( "IN_UCHOME" ) )
{
	...
	$actives 	= 	array('friend'=>' class="active"');
	$perpage 	= 	10;
	$mpurl 		= 	'ND_upload.php';
	...
	if(!empty($friends))
	{
		$sql 		= 	"select nd.*,nt.name as ntt from ".tname('nd_data')." as nd left join ".tname('nd_type')." as nt on nt.id =nd.type where nd.state>0 and nd.uid in (".$friends.")  order by nd.".$orders." ".$dec;
		$query 		= 	$_SGLOBAL['db']->query($sql);
...
include_once template("ND_upload/template/friend");
?>
