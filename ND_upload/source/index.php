...
$actives 	= 	array('nd_all'=>' class="active"');
$direct		=	$_GET['direct'];
$dec		=	'desc';
switch ($direct)
	{
...
$sql 		= 	"select nd.*,nt.name as ntt from ".tname('nd_data')." as nd left join ".tname('nd_type')." as nt on nt.id =nd.type where nd.state>0 order by nd.".$orders." ".$dec;
$query 		= 	$_SGLOBAL['db']->query($sql);
...
include_once template("ND_upload/template/index");
?>
