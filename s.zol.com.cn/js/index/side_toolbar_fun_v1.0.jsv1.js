			...
			ShopCart : function(data){  
				if(data && jQuery.isArray(data)){
					...
					function goodsList(goods,merId){
						...
						jQuery.each(goods,function(i,goods){
                                                    ...
                                                    if(goods.buyNumber == '1'){
                                                        minusNoclick    = 'zc-minus-noclick';
                                                    }
                                                    if(goods.buyNumber == goods.canBuyMaxNum){
                                                        plusNoclick     = 'zc-plus-noclick';
                                                    }
				    		...
				    		goodsListString += '<div class="zc-order-detail clearfix">\
								...
								<div class="zp-number"><em class="zc-minus '+minusNoclick+'">-</em><span data-max="'+ goods.canBuyMaxNum +'">'+ goods.buyNumber +'</span><em class="zc-plus '+plusNoclick+'">+</em></div>\
					...
					jQuery.each(data,function(index,item){
						complete += '<li><form action="../order.zol.com/index.php@c=ConfirmOrder" method="POST" data-totalPrice="0" target="_blank">\
							<div class="zc-shop-name"><input name="merchantId" type="checkbox" value="' + item.merId + '"><a href="../www.zol.com/shop_'+ item.merId + 'default.htm">' + item.merName + '</a></div>'+ goodsList(item.goodsInfo,item.merId) +'\
					...
					function same(item){
						return '<li data-id="'+ item.goodsId +'">\
							<a href="'+ item.goodsUrl +'" class="zc-pic" title="'+ item.goodsName +'" target="_blank">\
								<span class="zc-browse-list"><img height="75" src="'+ item.indexPicUrl +'" alt="' + item.goodsName + '"></span>\
